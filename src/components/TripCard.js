...
class TripCard extends React.Component {
  ...
  getActivities(id) {
    fetch(
      `https://tripr-backend.herokuapp.com/api/v1/destinations/${id}/activities`
    )
      .then(resp => resp.json())
        ...
        <div>
          <Item className="trip-card-item">
            ...
            <div className="trip-card">
              ...
              <Popup
                trigger={
                  <Button icon onClick={this.addActivity}>
                    Add an Activity For {this.state.destination.name}
                  </Button>
                }
                flowing
                on="click"
                  ...
                  {this.state.activities.map(activity => (
                    <Grid.Column textAlign="center">
                      ...
                      <div>
                        <Button
                          onClick={() => {
                            this.assignActivity(this.props.id, {
              ...
              <Item.Content className="trip-card-content">
                <Item.Image
                  onClick={this.clickChangeState}
                  size="medium"
                  src={this.state.destination.thumbnail}
                />

                <Item.Description className="trip-card-description">
                  <span id="trip-card-destination-name">
                    {this.state.destination.name}
                  </span>{' '}
                  <div>
                    {new Date(this.props.start_date).toLocaleDateString(
                      'en-US',
                      DATE_OPTIONS
                    )}
                    <p />
          ...
          {this.state.showActivities ? (
            <ActivitiesContainer
              url={`https://tripr-backend.herokuapp.com/api/v1/trips/${
                this.props.id
              }/activities`}
            />
          ) : null}
